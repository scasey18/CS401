...
// Global vtable for rectangles with the appropriate methods 
...
double getAreaRect(const void* _ths)	
{	   	
    Rectangle* const ths = (Rectangle*)_ths; 
    return (ths->top - ths->bottom) * (ths->right - ths->left);	   	
}	  

double getXposRect(const void* _ths)	   	
{	 
    Rectangle* const ths = (Rectangle*)_ths; 
    return (ths->left + ths->right)/2.0;
}	   	
 
double getYposRect(const void* _ths)  	
{ 
    Rectangle* const ths = (Rectangle*)_ths; 
    return (ths->top + ths->bottom)/2.0; 
} 

void scaleByRect(void* _ths, double factor)
{
    Rectangle* const ths = (Rectangle*)_ths; 
    double xc = vtable(ths)->getXpos(_ths);	   	
    double yc = vtable(ths)->getYpos(_ths);
...
void movePosRect(void* _ths, double dx, double dy)	   	
{
    Rectangle* const ths = (Rectangle*)_ths; 
    ths->top += dy;	  
    ths->bottom += dy; 
...
ShapeVTable rect_vtable(&getAreaRect, &getXposRect,
                             &getYposRect, &scaleByRect,	   	
                             &movePosRect);
                             
Rectangle* newRectangle(double x0, double y0, double x1, double y1)	  
{	
	Rectangle* ths = new Rectangle();	 
    ths->vtableptr = &rect_vtable;
	if (x0 < x1)	   	
    {
        ths->left = x0;	   	
        ths->right = x1;	  
